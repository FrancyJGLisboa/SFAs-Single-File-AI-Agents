...
"""
Chat App Single File Agent

Example Usage:
    uv run chat_app.py -d <data_path> -p "your analysis request" -c 5
"""
...
def pre_execution_validation(data: Dict, reasoning: str) -> bool:
    ...
    try:
        ...
        with MemoryManager() as mm:
            for check in ['prompt_validation', 'model_validation', 'cost_estimation']:
...
def during_execution_validation(data: Dict, reasoning: str) -> bool:
    ...
    try:
        ...
        with MemoryManager() as mm:
            for check in ['output_monitoring', 'cost_tracking', 'performance_monitoring']:
                if not globals()[f"validate_{check}"](data, f"Running {check}"):
                    return False
            return True
    except Exception as e:
...
def post_execution_validation(data: Dict, reasoning: str) -> bool:
    ...
    try:
        ...
        with MemoryManager() as mm:
            for check in ['response_validation', 'safety_check', 'quality_metrics']:
                if not globals()[f"validate_{check}"](data, f"Running {check}"):
                    return False
            return True
    except Exception as e:
...
def validate_prompts(reasoning: str, {{params}}) -> str:
    """{{description}}
    ...
    try:
        ...
        console.log(f"[blue]Strict Validation[/blue] - validate_prompts - Reasoning: {reasoning}")
        return result
    except ValidationError as e:
        console.log(f"[red]Schema validation failed: {str(e)}[/red]")
        return str(e)
    
def test_completions(reasoning: str, {{params}}) -> str:
    """{{description}}
    Implements strict validation with schema enforcement.
    """
    try:
        ...
        console.log(f"[blue]Strict Validation[/blue] - test_completions - Reasoning: {reasoning}")
        return result
    except ValidationError as e:
        console.log(f"[red]Schema validation failed: {str(e)}[/red]")
        return str(e)
    
def monitor_costs(reasoning: str, {{params}}) -> str:
    """{{description}}
    Implements strict validation with schema enforcement.
    """
    try:
        ...
        console.log(f"[blue]Strict Validation[/blue] - monitor_costs - Reasoning: {reasoning}")
        return result
    except ValidationError as e:
        console.log(f"[red]Schema validation failed: {str(e)}[/red]")
        return str(e)
